...
| `/run` | Operator screen — load/start/abort heats (start warns if any pilot has no timer registered), live M:SS.T countdown (tenths, 20fps), flight log with altitude and jumped-start notes, CD skip, competition filter chips (show all, or up to two competitions in side-by-side queue columns), dual F3K/F5K discipline columns otherwise, mark heats done/undone, auto-advance 3s toast, readiness check warning, timer connection status strip (T1/T2 pills), pilot status strip (○ unbound → ✓+T#), CD override form to manually log a flight for any pilot. **🔊 Callup** toggles the pre-prep announcement for the loaded heat only — an amber dot marks a heat that is not following the Settings default, and the next heat loaded returns to it either way. **✕ Clear** puts a loaded heat down without running it — distinct from **Abort**, which keeps it loaded so a round that went wrong can be restarted; before this the only way to clear a heat was to load a different one. A **firmware banner** names any connected timer running an older build, says whether it will update on its own or is being held by the loaded heat, and offers **Update now** |
...
| `/import` | Upload GliderScore `.mdb`, pick competition, import pilots/rounds/draw. Also takes a **zip of the GliderScore `Audio` folder** — the offline twin of the live audio sync in `F3KSync`; only new or changed files are written, so re-uploading the whole folder each event is cheap and expected |
...
**Audio library sync:** the same tool also copies any audio file the base station does
not already have from `<GliderScore>\Audio`. GliderScore generates the whole pack —
pilot names as `Surname_Firstname.wav` (its own synthesiser, or recorded, or typed
phonetically), plus `Round1-30`, `Group1-20` and the task descriptions. ⚠ **Pilot name
files are regenerated per competition**, so a base station that was current last weekend
is missing everyone who has joined since; the base owns the speaker, so the files have to
live there. Matched on name **and size**, so a re-recorded name replaces the old one.
Runs after the score sync and never blocks it (`--no-audio` opts out). `/import` accepts a
zip of the same folder for a CD working offline, and `GET /api/audio/pilot-coverage`
reports which pilots have no name file — worth checking before an event rather than
discovering it at the ready box.

⚠ Whatever GliderScore's synthesiser read when it built a file is what gets played. A
roster marked up with `ZZ` prefixes will have those letters **inside** the audio; matching
ignores the marker on both sides, but only regenerating the pack from clean names stops it
being spoken.
